<template>
  <Transition name="slide-down">
    <div 
      v-if="isOffline || justCameOnline" 
      class="offline-indicator"
      :class="{
        'offline': isOffline,
        'online': justCameOnline
      }"
    >
      <div class="flex items-center justify-center gap-2 px-4 py-2">
        <div v-if="isOffline" class="flex items-center gap-2">
          <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636a9 9 0 010 12.728m0 0l-2.829-2.829m2.829 2.829L21 21M15.536 8.464a5 5 0 010 7.072m0 0l-2.829-2.829m-4.243 2.829a4.978 4.978 0 01-1.414-2.83m-1.414 5.658a9 9 0 01-2.167-9.238m7.824 2.167a1 1 0 111.414 1.414m-1.414-1.414L3 3m8.293 8.293l1.414 1.414" />
          </svg>
          <span class="text-sm font-medium">No Internet Connection</span>
        </div>
        <div v-else-if="justCameOnline" class="flex items-center gap-2">
          <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span class="text-sm font-medium">Back Online</span>
        </div>
      </div>
    </div>
  </Transition>
</template>

<script setup lang="ts">
const { isOffline, justCameOnline } = useOffline()
</script>

<style scoped>
.offline-indicator {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
  z-index: 9999;
  transition: all var(--transition-base);
}

.offline-indicator.offline {
  background: var(--color-danger);
  color: white;
}

.offline-indicator.online {
  background: var(--color-success);
  color: white;
}

.slide-down-enter-active,
.slide-down-leave-active {
  transition: all var(--transition-base);
}

.slide-down-enter-from {
  transform: translateY(-100%);
  opacity: 0;
}

.slide-down-leave-to {
  transform: translateY(-100%);
  opacity: 0;
}
</style>
