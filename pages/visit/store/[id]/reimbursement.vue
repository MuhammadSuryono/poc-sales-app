<template>
  <div class="min-h-screen bg-gray-50">
    <CardNavbarBack title="Reimbursement Toko"/>
    <div class="max-w-[1000px] mx-auto px-4 sm:px-6 py-6 space-y-6">
      <div class="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h1 class="text-lg sm:text-xl font-bold text-gray-900">Reimbursement</h1>
            <p class="text-xs text-gray-500">Ajukan biaya kunjungan untuk Customer #{{ customerId }}</p>
          </div>
        </div>
      </div>
        ...
        <div class="lg:col-span-6">
          <div class="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm">
            <div class="flex items-center gap-2 mb-3">
              <button v-for="t in tabs" :key="t.key" class="px-3 py-2 rounded-xl text-xs font-bold border transition"
...
import type { ReimbursementStore } from '~/types/user'
...
const reimbursements = computed<ReimbursementStore[]>(() => userStore.reimbursementsStore)
...
const submit = async () => {
  ...
  await userStore.addStoreReimbursement(userStore.user?.user_id || '', {
    date: form.date,
    category: form.category as ReimbursementCategory,
    amount: form.amount as number,
    note: form.note,
    status: 'WAITING',
    customer_no: customerId.value,
...
const filtered = computed(() => {
  const base = reimbursements.value.filter(r => r.customer_no === customerId.value)
  if (activeTab.value === 'ALL') return base
  return base.filter(r => r.status === activeTab.value)
})
...
const labelStatus = (s: ReimbursementStatus) => {
  switch (s) {
    case 'WAITING': return 'Dalam Pengajuan'
